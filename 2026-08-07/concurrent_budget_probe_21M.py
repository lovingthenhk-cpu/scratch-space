"""
2026-08-07: real CONCURRENT 2-process memory verification for a further
MEM_ITEM_BUDGET 18,000,000 -> 21,000,000 raise, following the exact same
incremental-probe methodology as 2026-08-05's concurrent_budget_probe.py
(one step at a time, always with a real 2-process measurement before
trusting extrapolation -- see 2026-08-05 PROGRESS.md note: "線形外挿だけで
25M以上に飛びつかず、実測データ点を1段階ずつ増やす方針を継続すること").

CONTEXT: today's run_production_streaming_v8.py already raised the actual
production MEM_ITEM_BUDGET to 18,000,000 for real (not just a probe) and
confirmed s=9 (->150), s=11 (->81), s=14 (->58), s=16 (->48) all reach
their new natural ceiling with 0 hits, real combined peak RSS ~3.82 GiB
(see memory_monitor_log.csv). This probe checks whether ANOTHER step, to
21,000,000, stays within a safe memory margin -- this is exploratory
groundwork for a *future* session's production run, not something this
session commits to a full sweep for (time budget for today already mostly
spent on the real 18M production run above).

METHOD: identical structure to 2026-08-05's probe -- recompute which of
the ten s in [9..18] (at THEIR ceiling as of TODAY's v8 run) would advance
further under NEW_BUDGET=21,000,000, then run the two candidates with the
...
OLD_BUDGET = 18_000_000
NEW_BUDGET = 21_000_000
PER_A_TIME_LIMIT = 900.0
EMPIRICAL_RATE = 1_000_000

# ceilings as of TODAY's (2026-08-07) v8 production run.
CEILINGS_18M = {9: 150, 10: 99, 11: 81, 12: 72, 13: 58, 14: 58, 15: 51, 16: 48, 17: 47, 18: 44}
STUCK = {s: A + 1 for s, A in CEILINGS_18M.items()}
...
if len(pair) < 2:
    ...
    with open("concurrent_budget_probe_21M_results.json", "w") as f:
...
with open("concurrent_budget_probe_21M_timeline.csv", "w") as f:
    keys = ["t", "combined_kb"] + [f"{s}_{A}" for s, A, _ in worker_paths]
    f.write(",".join(keys) + "\n")
    for row in samples:
        f.write(",".join(str(row.get(k, "")) for k in keys) + "\n")

print("\n=== results written to concurrent_budget_probe_21M_results.json / _timeline.csv ===", flush=True)
